                    ...
                    <div class="card card-uni-11 event-card h-100 d-flex flex-column">
                        ...
                        <a href="{{ url('event-detail/' . $event->slug) }}">
                            <img src="{{ asset('assets/admin/uploads/events/' . $event->image) }}"
                                class="event-img-sm mb-3" alt="{{ $event->title }}">
                        </a>
                        @else
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="event-date small text-muted">
                                ...
                                @if ($daysDiff <= 7) {{ $eventDate->format('l') }} •
                                    {{ \Carbon\Carbon::parse($event->event_time)->format('h:i A') }} @elseif($daysDiff <
                                    28) {{ ceil($daysDiff / 7) }} week{{ ceil($daysDiff / 7) > 1 ? 's' : '' }} later
                                    @else {{ ceil($daysDiff / 30) }} month{{ ceil($daysDiff / 30) > 1 ? 's' : '' }}
                                    later @endif </span>
                        </div>

                        <h5 class="fw-bold mb-1">
                            <a href="{{ url('event-detail', $event->slug) }}" class="text-decoration-none"
                                style="color:#0f766e;">
                                {{ $event->title }}
                            </a>
                        </h5>
                        ...
                        <div class="main-view-btn">
                            <a href="{{ url('event-detail', $event->slug) }}"
                                class="btn btn-outline-uni btn-sm px-3 mainevent-view-btn">
                                View details
                            </a>
